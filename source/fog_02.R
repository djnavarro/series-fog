...
smudged_hexagon <- function(seed, noise1 = 0, noise2 = 2, noise3 = .5) {
  set.seed(seed)
...
art_generator <- function(seed) {
  ...
  for(i in 1:n_hex) {
    dat[[i]] <- smudged_hexagon(seed = hex_seeds[i]) |>
      dplyr::mutate(
        fill = shades[i], 
        s = dplyr::if_else(i == 1, 2, stats::runif(n = 1, min = 1, max = 4)),
        x = x * s + dplyr::if_else(i == 1, 0, stats::runif(n = 1, min = -.1, max = .1)), 
        y = y * s + dplyr::if_else(i == 1, 0, stats::runif(n = 1, min = -.1, max = .1)),
        height = stats::runif(1),
