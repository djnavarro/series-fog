...
art_generator <- function(seed) {
  ...
  for(i in 1:n_hex) {
    dat[[i]] <- smudged_hexagon(seed = hex_seeds[i]) |>
      dplyr::mutate(
        ...
        x = x * s + dplyr::if_else(i == 1, 0, stats::runif(n = 1, min = -w, max = w)), 
        y = y * s + dplyr::if_else(i == 1, 0, stats::runif(n = 1, min = -w, max = w)),
  ...
  ggplot2::ggsave(
    ...
    units = "px",
    dpi = 300,
    bg = sample(c("#222222", "#dddddd"), 1L) #shades[length(shades)]
  )  
