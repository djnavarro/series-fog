...
art_generator <- function(seed) {
  ...
  queers <- c("#e50000", "#ff8d00", "#ffee00", "#028121", "#004cff", "#770088")
  hex_shade <- sample(queers, n + 1, TRUE)
  hex_seed <- sample(1:10000, n)
  hex_size <- runif(n, min = 1.5, max = 1.5)
  hex_noise <- runif(n, min = 1, max = 2)
  bg <- "#000000"
  ...
  y_edge <- (n_row - 1 + sqrt(2))/2
  x_edge <- (n_col - 1 + sqrt(2))/2
